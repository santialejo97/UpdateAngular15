...
export class IncapacidadContingenciaComponent implements OnInit {
  ...
  get f_fechaExpedicion() { return this.fechaExpedicionForm.controls; }

  get f_profesional() { return this.profesionalForm.controls; }
  ...
  async ngOnInit() {
    ...
    this.consultaPlanContingenciaActual  = await this.consultarPlanContingenciaEnEjecucion();

    this.Alertas.alertaInformativa('', 'Solo se debe hacer uso de esta funcionalidad si se realizó una incapacidad en SIPE local offline por contingencia', 'Cerrar');
  ...
  async consultarPlanContingenciaEnEjecucion() {
    let valuesParam = {
      fecha_registro : this.f_fechaExpedicion.fechaExpedicion.value == '' ? null : this.f_fechaExpedicion.fechaExpedicion.value, 
      id_entidad: this.perfilUbicacion.idEntidad
    };
    const result = await this.parametrosIncapacidadesService.postIncapacidad('/Incapacidad/ConsultaPlanContingencia', valuesParam);
    console.log(result)
    return result;
  }
  ...
  async onSubmitFechaForm() {
      ...
      if (this.fechaExpedicionForm.valid) {
          ...
          const consultaPlanContingenciaEjecucion  = await this.consultarPlanContingenciaEnEjecucion();
          console.log(consultaPlanContingenciaEjecucion)

          if(consultaPlanContingenciaEjecucion == null){
              this.Alertas.alertaInformativa('', 'La entidad no presenta registro de plan de contingencia activo', 'Cerrar');
          }else{
  ...
  async onSubmitProfesionalForm(){
      ...
      if (this.profesionalForm.valid) {
          const usuarioEncontrado = await this.usuarioFuncionesService.post('ConsultarUsuarioPorDocumento', 
                                                    { 
                                                      'tipoDocumento' : this.f_profesional.tipoDocumento.value, 
                                                      'numeroDocumento' : this.f_profesional.numeroDocumento.value, 
                                                      'idEntidad' : this.perfilUbicacion.idEntidad
                                                    });
          if(usuarioEncontrado != null){
              await this.Alertas.alertaInformativa('', 'Profesional de la salud Verificado Correctamente', 'Cerrar');
              this.next();
          }else{
              await this.Alertas.alertaInformativa('', 'Profesional en la salud no tiene permisos o no se encontró', 'Cerrar');
              return;
          }
  ...
  expedirIncapacidadContingencia() {
    ...
    fechaValida.setMinutes(fechaValida.getMinutes() + environment.TiempoDeValidezLinkEnMinutos);

    let mensajeRegistrarIncapacidad = {
      id: "0",
      idPlanContingencia : this.idPlanContingenciaLink,
      fechaExpedicionForm:  this.f_fechaExpedicion.fechaExpedicion.value,
      fechaValidacion: fechaValida,
      idUsuarioEntidadRol: this.perfilUbicacion.idUsuarioEntidadRol
    };

    const secretKey = this._accountService.userValue.access_token.substring(0,16);
    const hash = this._cryptoService.getHash(mensajeRegistrarIncapacidad,secretKey);
