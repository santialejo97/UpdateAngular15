...
export class EditarPacienteComponent implements OnInit {
  ...
  createPacienteForm() {
    ...
    this.pacienteForm = this.formBuilder.group({
      tipo_documento: [
        { value: this.pacienteIn.tipo_documento, disabled: true },
      ],
      numero_documento: [
        { value: this.pacienteIn.numero_documento, disabled: true },
      ],
      primer_nombre: [
        { value: this.pacienteIn.primer_nombre, disabled: this.deshabilitar },
        Validators.required,
      ],
      segundo_nombre: [
        { value: this.pacienteIn.segundo_nombre, disabled: this.deshabilitar },
      ],
      primer_apellido: [
        { value: this.pacienteIn.primer_apellido, disabled: this.deshabilitar },
        Validators.required,
      ],
      segundo_apellido: [
        {
          value: this.pacienteIn.segundo_apellido,
          disabled: this.deshabilitar,
        },
      ],
      sexo: [
        { value: this.pacienteIn.sexo, disabled: this.deshabilitar },
        Validators.required,
      ],
      ...
      fecha_nacimiento: [
        {
          value: this.pacienteIn.fecha_nacimiento,
          disabled: this.deshabilitar,
        },
        Validators.required,
      ],
      cod_depto_residencia: [
        {
          value: this.pacienteIn.cod_depto_residencia,
          disabled: this.deshabilitar,
        },
        Validators.required,
      ],
      cod_mun_residencia: [
        {
          value: this.pacienteIn.cod_mun_residencia,
          disabled: this.deshabilitar,
        },
        Validators.required,
      ],
      id_regimen: [
        { value: this.pacienteIn.id_regimen, disabled: this.deshabilitar },
        Validators.required,
      ],
      regimen_descripcion: [''],
      eps: [
        { value: this.pacienteIn.eps, disabled: this.deshabilitar },
        Validators.required,
      ],
  ...
  ActualizarValoresdeHabilitacion() {
    if (this.deshabilitar == false) {
      this.pacienteForm.get('tipo_documento')?.enable();
      this.pacienteForm.get('numero_documento')?.enable();

      this.pacienteForm.get('primer_nombre')?.enable();
      this.pacienteForm.get('segundo_nombre')?.enable();
      this.pacienteForm.get('primer_apellido')?.enable();
      this.pacienteForm.get('segundo_apellido')?.enable();
      this.pacienteForm.get('sexo')?.enable();
      this.pacienteForm.get('fecha_nacimiento')?.enable();

      this.pacienteForm.get('id_regimen')?.enable();
      this.pacienteForm.get('eps')?.enable();
      ...
      this.pacienteForm.get('cod_depto_residencia')?.enable();
      this.pacienteForm.get('cod_mun_residencia')?.enable();
    } else {
      this.pacienteForm.get('tipo_documento')?.disable();
      this.pacienteForm.get('numero_documento')?.disable();

      this.pacienteForm.get('primer_nombre')?.disable();
      this.pacienteForm.get('segundo_nombre')?.disable();
      this.pacienteForm.get('primer_apellido')?.disable();
      this.pacienteForm.get('segundo_apellido')?.disable();
      this.pacienteForm.get('sexo')?.disable();
      this.pacienteForm.get('fecha_nacimiento')?.disable();

      this.pacienteForm.get('id_regimen')?.disable();
      this.pacienteForm.get('eps')?.disable();
      ...
      this.pacienteForm.get('cod_depto_residencia')?.disable();
      this.pacienteForm.get('cod_mun_residencia')?.disable();
    }
  ...
  async ActualizarValoresFormulario(pac: IPacienteModel) {
    ...
    if (this.listaMunicipios) {
      this.listaMunicipiosByDepto = this.listaMunicipios.filter(
        (x: any) =>
          x.cod_depto == this.pacienteForm.get('cod_depto_residencia')?.value
      );
    }

    //this.loggingService.registroLog(this.f_paciente.id_regimen.value );
    if (this.pacienteForm.get('id_regimen')?.value) {
      this.listaAdministradorasByCodRegimen =
        await this.parametrosIncapacidadesService.get(
          '/Administradoras?codRegimen=' +
            this.pacienteForm.get('id_regimen')?.value +
            '&tipoAdministradora='
        );
    }
 ...
 /*********** Eventos selects ********/
  ...
  async abrirModalConfirmacionPaciente(modeloPaciente: IPacienteModel) {
    ...
    if (minDate.getTime() > modeloPaciente.fecha_nacimiento.getTime()) {
      await this.Alertas.alertaInformativa(
        'Advertencia',
        'La fecha de nacimiento no puede ser anterior a ' +
          this.getFormattedDateEstandar(this.getFormattedDate(minDate)),
        'Cerrar'
      );
      return;
    }
    ...
    let RegimenEncontrado = this.listaRegimen.find(function (element: any) {
      return element.cod_regimen == modeloPaciente.id_regimen;
    });

    let sexoEncontrado = this.listaSexo.find(function (element: any) {
      return element.cod_sexo == modeloPaciente.sexo;
    });

    let ciudadEncontrado = this.listaMunicipios.find(function (element: any) {
      return (
        element.cod_municipio == modeloPaciente.cod_mun_residencia &&
        element.cod_depto == modeloPaciente.cod_depto_residencia
      );
    });
    ...
    let administradoraEncontrada = this.listaAdministradorasByCodRegimen.find(
      function (element: any) {
        return element.cod_administradora == modeloPaciente.eps;
      }
    );

    this.pacienteIn.regimen_descripcion =
      RegimenEncontrado != null ? RegimenEncontrado.descripcion : '';
    this.pacienteIn.sexo_descripcion =
      sexoEncontrado != null ? sexoEncontrado.descripcion : '';
    this.pacienteIn.ciudad_residencia_descripcion =
      ciudadEncontrado != null ? ciudadEncontrado.nom_ciudad : '';
    this.pacienteIn.depto_residencia_descripcion =
      departamentoEncontrado != null ? departamentoEncontrado.nom_depto : '';
    this.pacienteIn.eps_nombre =
      administradoraEncontrada != null
        ? administradoraEncontrada.razon_social
        : '';

    this.pacienteIn.numero_documento =
      this.pacienteForm.get('numero_documento')?.value;
    this.pacienteIn.tipo_documento =
      this.pacienteForm.get('tipo_documento')?.value;
    this.pacienteIn.primer_nombre = this.pacienteForm
      .get('primer_nombre')
      ?.value.trim();
    this.pacienteIn.segundo_nombre = this.pacienteForm
      .get('segundo_nombre')
      ?.value.trim();
    this.pacienteIn.primer_apellido = this.pacienteForm
      .get('primer_apellido')
      ?.value.trim();
    this.pacienteIn.segundo_apellido = this.pacienteForm
      .get('segundo_apellido')
      ?.value.trim();

    let fechaNacimientoAux = new Date(
      this.pacienteForm.get('fecha_nacimiento')?.value
    );
    fechaNacimientoAux.setDate(fechaNacimientoAux.getDate());
    this.pacienteIn.fecha_nacimiento = fechaNacimientoAux;
    this.pacienteIn.fecha_nacimiento_string =
      this.getFormattedDate(fechaNacimientoAux);

    this.pacienteIn.eps = this.pacienteForm.get('eps')?.value;

    this.pacienteIn.sexo = this.pacienteForm.get('sexo')?.value;
    this.pacienteIn.cod_depto_residencia = this.pacienteForm.get(
      'cod_depto_residencia'
    )?.value;
    this.pacienteIn.cod_mun_residencia =
      this.pacienteForm.get('cod_mun_residencia')?.value;
    this.pacienteIn.id_regimen = this.pacienteForm.get('id_regimen')?.value;
    ...
    modalRef.componentInstance.dataIn = {
      name: '',
      id: 0,
      incapacidad_data: this.pacienteForm.value,
      paciente_data: this.pacienteIn,
    };
  ...
  public changeDepartamentosConsulta(e: any) {
    this.listaMunicipiosByDepto = this.listaMunicipios.filter(
      (x: any) =>
        x.cod_depto == this.pacienteForm.get('cod_depto_residencia')?.value
    );
  ...
  public changeMunicipiosConsulta(e: any) {
    this.loggingService.registroLog('change select');
  }
  ...
  public async changeRegimen(e: any) {
  ...
  async cargarListas() {
    ...
    this.listaRegimen = await this.parametrosIncapacidadesService.get(
      '/regimen'
    );
