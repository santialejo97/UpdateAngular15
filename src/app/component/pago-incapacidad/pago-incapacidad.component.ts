...
export class pagoIncapacidadComponent implements OnInit {
  ...
  get f_personaIncapacidad() {
    return this.formPersonaIncapacidad.controls;
  }
  ...
  get f_aportante() {
    return this.formAportante.controls;
  }
  ...
  get f_pago() {
    return this.formInformePagoIncapacidad.controls;
  }
  ...
  deshabilitarValidaciones() {
    if (this.f_personaIncapacidad['numeroIncapacidad'].value != '') {
      ...
      this.f_personaIncapacidad['numeroIncapacidad'].setValidators(
        Validators.required
      );
    } else if (this.f_personaIncapacidad['numeroDocumento'].value != '') {
      this.f_personaIncapacidad['tipoDocumento'].setValidators(
        Validators.required
      );
      this.f_personaIncapacidad['numeroDocumento'].setValidators(
        Validators.required
      );
  ...
  async validarDatosAportante() {
    ...
    if (this.formAportante.invalid) {
      ...
      let result = await this.parametrosIncapacidadesService.postIncapacidad(
        '/RegistrarPago/ConsultarPagos',
        pagoValue
      );
      if (result.length > 0) {
        await this.Alertas.alertaInformativa(
          'Advertencia',
          'La incapidad ya tien un pago reportado para este aportante',
          'Cerrar'
        );
        return;
      }
      ...
      console.log(formAportanteData.tipoDocumentoAportante);
      this.f_pago['tipoDocumentoAportante'].setValue(
        formAportanteData.tipoDocumentoAportante
      );
      this.f_pago['numeroDocumentoAportante'].setValue(
        formAportanteData.numeroDocumentoAportante
      );
      this.f_pago['razonSocialAportante'].setValue(
        formAportanteData.razonSocialAportante
      );
      this.next();
    }
  }
    ...
    try {
      let valuesParam = {
        ...
        numero_documento_pac:
          this.f_personaIncapacidad['numeroDocumento'].value,
      ...
      for (let incElement of this.listaIncapacidadesAnteriores) {
        ...
        let result = await this.parametrosIncapacidadesService.postIncapacidad(
          '/ConsultarRegistroPago/ConsultarRegistroPago',
          valuesParamQuery
        );
  ...
  async buscarInformeIncapacidad(incapacidadIndex: any) {
    ...
    try {
      if (
        this.listaIncapacidadesAnteriores[incapacidadIndex]
          .id_estado_pago_validar == 2
      ) {
        await this.Alertas.alertaInformativa(
          'Advertencia',
        ...
        this.listaIncapacidadesAnteriores[incapacidadIndex]
          .id_estado_pago_validar == 3
      ) {
        await this.Alertas.alertaInformativa(
          'Advertencia',
          'Incapacidad se encuentra anulada no procede pago',
          'Cerrar'
        );
        return;
      } else if (
        this.listaIncapacidadesAnteriores[incapacidadIndex]
          .id_estado_pago_validar == 4
      ) {
        await this.Alertas.alertaInformativa(
          'Advertencia',
          'El paciente no pertenece al régimen contributivo, especial o de excepción, o subsidiado con afiliación a ARL',
          'Cerrar'
        );
        return;
      } else if (
        this.listaIncapacidadesAnteriores[incapacidadIndex]
          .id_estado_pago_validar == 5
      ) {
        await this.Alertas.alertaInformativa(
          'Advertencia',
          'La incapacidad no tiene notificación creada',
          'Cerrar'
        );
        return;
      }
      ...
      const incapacidad: any = {
        numeroIncapacidad:
          this.listaIncapacidadesAnteriores[incapacidadIndex].id_incapacidad,
        tipoDocumento:
          this.listaIncapacidadesAnteriores[incapacidadIndex]
            .tipo_documento_pac,
        numeroDocumento:
      ...
      this.listaRelacionPacienteAportante =
        await this.parametrosIncapacidadesService.getIncapacidad(
          '/RelacionPacienteAportante/' + incapacidad.numeroIncapacidad
        );

      this.parametrosIncapacidadesService
        .getIncapacidadAnular(
          '/Incapacidad',
          incapacidad.numeroIncapacidad,
          incapacidad.tipoDocumento,
        ...
        .subscribe(
          (data) => {
            ...
            if (this.incapacidadResumen.notificacionRadicacion) {
              this.f_pago['fechaRadicacionInc'].setValue(
                this.getFormattedDateTime(
                  new Date(
                    this.incapacidadResumen.notificacionRadicacion.fecha_notificacion
                  )
                )
              );
            }
            ...
            if (
              this.listaRelacionPacienteAportante != null &&
              this.listaRelacionPacienteAportante.length > 0
            ) {
              this.next();
  ...
  changeDiasPago() {
    ...
    if (
      this.f_pago['diasPago'].value < this.incapacidadResumen.dias_incapacidad
    ) {
  ...
  async cargarListas() {
    try {
      ...
      this.listaCausalGlosa = await this.parametrosIncapacidadesService.get(
        '/causalglosa'
      );
      console.log('listaCausalGlosa cargada con exito');

      this.listaTipoPago = await this.parametrosIncapacidadesService.get(
        '/tipopago'
      );
      console.log('listaTipoPago cargada con exito');

      this.listaEstadoPago = await this.parametrosIncapacidadesService.get(
        '/estadopago'
      );
      console.log('listaEstadoPago cargada con exito');

      this.listaCausalDias = await this.parametrosIncapacidadesService.get(
        '/causaldias'
      );
  ...
  async validarDatosPago() {
    ...
    let elemento_estado_pago = this.listaEstadoPago.find(function (
      element: any
    ) {
      return element.id_estado_pago == idEstadoPago;
    });

    let elemento_causal_glosa = this.listaCausalGlosa.find(function (
      element: any
    ) {
    ...
    let elemento_causal_dias = this.listaCausalDias.find(function (
      element: any
    ) {
    ...
    const perfilUbicacion: IPerfilComple =
    ...
    this.pagoModel = {
      ...
      causal_glosa:
        this.pagoForm.causalGlosa != null && this.pagoForm.causalGlosa != ''
          ? parseInt(this.pagoForm.causalGlosa)
          : 0,
      nro_autorizacion: this.pagoForm.numeroAutorizacion
        ? this.pagoForm.numeroAutorizacion
        : '',
      tipo_pago: parseInt(this.pagoForm.tipoPago),
      valor_pagado: this.pagoForm.valorPago
        ? parseFloat(this.pagoForm.valorPago.replace('$', '').replace('.', ''))
        : 0,
      dias_pagados: parseInt(
        this.pagoForm.diasPago ? this.pagoForm.diasPago : 0
      ),
      causal_dias:
        this.pagoForm.causalDiasPago != null &&
        this.pagoForm.causalDiasPago != ''
          ? parseInt(this.pagoForm.causalDiasPago)
          : 0,
      fecha_pago: this.pagoForm.fechaPago
        ? this.pagoForm.fechaPago
        : this.getFormattedDateTime(new Date()),
      ibc_pago: this.pagoForm.ingresoBaseCotizacion
        ? parseFloat(
            this.pagoForm.ingresoBaseCotizacion
              .replace('$', '')
              .replace('.', '')
          )
        : 0,
      ...
      descripcion_tipo_pago: elemento_tipo_pago
        ? elemento_tipo_pago.descripcion
        : '',
      descripcion_causal_glosa: elemento_causal_glosa
        ? elemento_causal_glosa.descripcion
        : '',
      descripcion_causal_dias: elemento_causal_dias
        ? elemento_causal_dias.descripcion
        : '',
  ...
  async guardarPago() {
    ...
    let result =
      await this.parametrosIncapacidadesService.guardarDatosPagoIncapacidad(
        this.pagoModel
      );
    ...
    if (result != null && result == '1') {
      ...
      this.Alertas.alertaInformativa(
        '',
        'Error al guardar el registro de pago',
        'Cerrar'
      );
    }
  }
  ...
  async openModalAportantes() {
    ...
    await modalRef.result.then(async (result) => {
      console.log(result);
      this.f_aportante['tipoDocumentoAportante'].setValue(
        result.tipo_documento_ap
      );
      this.f_aportante['numeroDocumentoAportante'].setValue(
        result.numero_documento_ap
      );
      this.f_aportante['razonSocialAportante'].setValue(
        result.razon_social_descripcion
      );
    });
  }
}
