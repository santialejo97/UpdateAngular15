...
export class PerdidaCapacidadLaboralComponent implements OnInit {
  ...
  createPerdidaCapacidadLaboralForm() {
    ...
    this.perdidaCapacidadLaboralForm = this.formBuilder.group({
      ...
      fechaRegistroCalificacion: [
        { value: this.getFormattedDate(currentDate), disabled: false },
      ],
      ...
      nombreRazonSocialEntidad: [
        { value: '', disabled: true },
        Validators.required,
      ],

      tipoDocumentoProfesionalEmite: [
        { value: '', disabled: true },
        Validators.required,
      ],
      numeroDocumentoProfesionalEmite: [
        { value: '', disabled: true },
        Validators.required,
      ],
      nombreProfesionalEmite: [
        { value: '', disabled: true },
        Validators.required,
      ],
    });
    ...
    try {
      ...
      this.listaConceptoRehabilitacion =
        await this.parametrosIncapacidadesService.postIncapacidad(
          '/ConsultaConceptoRehabilitacion/ConsultarListaCR',
          this.pacienteGet
        );
  ...
  async abrirListaDePerdidaCapacidad(idConcepto: any) {
    ...
    this.listaPedidadCapacidadLaboralPorConcepto =
      await this.parametrosIncapacidadesService.consultarPerdidaCapacidadLaboralPorIdConcepto(
        idConcepto
      );
  ...
  async abrirVerInformacionPerdida(idPCL: any) {
    for (let pclBusqueda of this.listaPedidadCapacidadLaboralPorConcepto) {
      if (pclBusqueda.numero_registro == idPCL) {
        ...
        this.f_PCL['fechaRegistroCalificacion'].patchValue(
          formatDate(
            new Date(pclBusqueda.fecha_registro_calificacion),
            'yyyy-MM-dd',
            'en'
          )
        );
        this.f_PCL['id_origen'].setValue(pclBusqueda.id_origen);
        this.f_PCL['porcentajePCL'].setValue(
          pclBusqueda.porcentaje_perdida_capacidad_laboral
        );
        this.f_PCL['fechaCalificacionPCL'].patchValue(
          formatDate(
            new Date(pclBusqueda.fecha_calid_incapacidadificacion_PCL),
            'yyyy-MM-dd',
            'en'
          )
        );
        this.f_PCL['fechaEstructuracion'].setValue(
          formatDate(
            new Date(pclBusqueda.fecha_estructuracion),
            'yyyy-MM-dd',
            'en'
          )
        );
        this.f_PCL['nuevaCalificacion'].setValue(
          pclBusqueda.nueva_calificacion
        );
        ...
        this.f_PCL['despachoAutoridadJudicial'].setValue(
          pclBusqueda.despacho_autoridad_judicial
        );
        this.f_PCL['nombreJuez'].setValue(pclBusqueda.nombres_apellidos_juez);

        this.f_PCL['tipoDocumentoEntidad'].setValue(
          pclBusqueda.tipo_documento_entidad
        );
        this.f_PCL['numeroDocumentoEntidad'].setValue(
          pclBusqueda.numero_documento_entidad
        );
        this.f_PCL['nombreRazonSocialEntidad'].setValue(
          pclBusqueda.nombre_razon_social
        );

        this.f_PCL['tipoDocumentoProfesionalEmite'].setValue(
          pclBusqueda.tipo_documento_emite
        );
        this.f_PCL['numeroDocumentoProfesionalEmite'].setValue(
          pclBusqueda.numero_documento_emite
        );
        this.f_PCL['nombreProfesionalEmite'].setValue(
          pclBusqueda.nombre_profesional_emite
        );
  ...
  async ObtenerValorNumeroRegistro() {
    const value =
      await this.parametrosIncapacidadesService.obtenerSiguienteValorSecuencia(
        'PCL'
      );
  ...
  async abrirModalConfirmacion(datosPerdidaIncapacidadLaboral: any) {
    ...
    this.pcl.fecha_registro_calificacion =
      datosPerdidaIncapacidadLaboral.fechaRegistroCalificacion;
    this.pcl.id_origen = Number(datosPerdidaIncapacidadLaboral.id_origen);
    this.pcl.porcentaje_perdida_capacidad_laboral = Number(
      datosPerdidaIncapacidadLaboral.porcentajePCL
    );
    this.pcl.fecha_calid_incapacidadificacion_PCL =
      datosPerdidaIncapacidadLaboral.fechaCalificacionPCL;
    this.pcl.fecha_estructuracion =
      datosPerdidaIncapacidadLaboral.fechaEstructuracion;
    this.pcl.nueva_calificacion = Number(
      datosPerdidaIncapacidadLaboral.nuevaCalificacion
    );
    this.pcl.observaciones = datosPerdidaIncapacidadLaboral.observacionesPCL;
    this.pcl.tipo_profesional = Number(
      datosPerdidaIncapacidadLaboral.tipoProfesional
    );

    this.pcl.despacho_autoridad_judicial =
      datosPerdidaIncapacidadLaboral.despachoAutoridadJudicial;
    this.pcl.nombres_apellidos_juez = datosPerdidaIncapacidadLaboral.nombreJuez;

    this.pcl.tipo_documento_entidad =
      datosPerdidaIncapacidadLaboral.tipoDocumentoEntidad;
    this.pcl.numero_documento_entidad =
      datosPerdidaIncapacidadLaboral.numeroDocumentoEntidad;
    this.pcl.nombre_razon_social =
      datosPerdidaIncapacidadLaboral.nombreRazonSocialEntidad;

    this.pcl.tipo_documento_emite =
      datosPerdidaIncapacidadLaboral.tipoDocumentoProfesionalEmite;
    this.pcl.numero_documento_emite =
      datosPerdidaIncapacidadLaboral.numeroDocumentoProfesionalEmite;
    this.pcl.nombre_profesional_emite =
      datosPerdidaIncapacidadLaboral.nombreProfesionalEmite;

    const elemetntFinded = this.listaPedidadCapacidadLaboralPorConcepto.find(
      (element: any) =>
        element.id_concepto_registro == this.idConceptoSeleccionado
    );
  ...
  async guardarPCL(pcl: IPerdidaCapacidadLaboral) {
    ...
    await modalRef.result.then(async (result) => {
      ...
      if (result.resultado == 'si') {
        ...
        let result =
          await this.parametrosIncapacidadesService.guardarDatosPerdidaCapacidadLaboralIncapacidad(
            pcl
          );
