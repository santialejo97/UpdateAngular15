...
export class ModalIncapacidadAnteriorComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.fechaInicioAnterior = new Date(
      this.dataIn.lista_incapacidades[0].fecha_inicio_string
    );
    this.fechaInicioAnterior.setDate(this.fechaInicioAnterior.getDate() + 1); //sumar 1 dia por la conversion

    this.fechaFinAnterior = new Date(
      this.dataIn.lista_incapacidades[0].fecha_fin_string
    );
    this.fechaFinAnterior.setDate(this.fechaFinAnterior.getDate() + 1); //sumar 1 dia por la conversion

    this.fechaInicioActual = new Date(
      this.dataIn.incapacidad_data.fecha_inicio_string
    );
    ...
    this.fechaFinActual = new Date(
      this.dataIn.incapacidad_data.fecha_fin_string
    );
    ...
    if (this.fechaFinActual.getTime() > this.fechaInicioNueva.getTime()) {
      var diff =
        this.fechaFinActual.getTime() - this.fechaInicioNueva.getTime();
    ...
    //traslape que no requiere calculo de fechas y dias (incapacidad que no acumula dias)
      ...
      this.nuevosDiasIncapacidad =
        this.dataIn.incapacidad_data.dias_incapacidad;
