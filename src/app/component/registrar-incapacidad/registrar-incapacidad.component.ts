...
import { IPerfil } from 'src/app/interfaces/IPerfil';
import { IRelacionPacienteAfiliacionSaludModel } from 'src/app/models/RelacionPacienteAfiliacionSalud.model';
// WS F
...
export class RegistrarIncapacidadComponent
  implements OnInit, AfterContentChecked
{
  //@Input() public IncapacidadIn:IIncapacidad;
  @Input() name!: string;

  @ViewChild(EditarPacienteComponent, { static: false })
  datosDesdeElPadre!: EditarPacienteComponent;
  ...
  incapacidadAnterior!: IIncapacidad | null;
  ...
  async ngOnInit() {
    ...
    if (this.route.snapshot.params['idIncapacidad'] != null) {
      let secretKey = this._accountService!.userValue!.access_token.substring(
        0,
        16
      );
      this.mensajeRegistroIdIncapacidad = JSON.parse(
        this._cryptoService.getEncriptMessage(
      ...
      if (
        this.mensajeRegistroIdIncapacidad?.id != null &&
        this.mensajeRegistroIdIncapacidad?.id != '0'
      ) {
  ...
  async validarTiempoHablitadolink() {
    const fechaValidacion = new Date(
      this.mensajeRegistroIdIncapacidad.fechaValidacion
    );
    ...
    if (fechaValidacion.getTime() < fechaH.getTime()) {
      await this.Alertas.alertaInformativa(
        'Advertencia',
        'El link para registrar la incapacidad ya no es válido',
        'Cerrar'
      );
  ...
  async cargarReexpedicion() {
    ...
    try {
      ...
      let camposAnulacion =
        await this.parametrosIncapacidadesService.getCamposAnulacion(
          '/Incapacidad',
      ...
      if (
        this.IncapacidadDetalles.id_incapacidad.indexOf('2', 0) ==
        -1 /*|| this.IncapacidadDetalles.id_incapacidad.anulada == false*/
      ) {
        this.loggingService.registroLog(
          'no encontrado' + this.IncapacidadDetalles.id_incapacidad
        );
      ...
      this.paciente!.id_incapacidad_anulado =
        this.mensajeRegistroIdIncapacidad?.id_incapacidad_anulado;
      ...
      this.incapacidadForm.patchValue({
        ...
        diagnostico_principal: !str.includes('"Diagnostico"')
          ? this.IncapacidadDetalles.diagnostico.id_diagnosticos
          : 0,
        codigo_diagnostico_principal: !str.includes('"Diagnostico"')
          ? this.IncapacidadDetalles.diagnostico.cod_diagnostico
          : '',
        diagnostico_relacion_1: !str.includes('"DiagnosticoRealacionUno"')
          ? this.IncapacidadDetalles.diagnosticoRelacionUno?.id_diagnostico_uno
          : 0,
        codigo_diagnostico_relacion_1: !str.includes(
          '"DiagnosticoRealacionUno"'
        )
          ? this.IncapacidadDetalles.diagnosticoRelacionUno
              ?.cod_diag_relacion_uno
          : '',
        diagnostico_relacion_2: !str.includes('"DiagnosticoRealacionDos"')
          ? this.IncapacidadDetalles.diagnosticoRelacionDos?.id_diagnostico_dos
          : 0,
        codigo_diagnostico_relacion_2: !str.includes(
          '"DiagnosticoRealacionDos"'
        )
          ? this.IncapacidadDetalles.diagnosticoRelacionDos
              ?.cod_diag_relacion_dos
          : '',
        id_servicio: !str.includes('"grupoServiciosId"')
          ? this.IncapacidadDetalles.grupoServicios.id_servicio
          : '',
        id_modalidad: !str.includes('"Modalidad"')
          ? this.IncapacidadDetalles.modalidad.id_modalidad
          : '',
        id_origen: !str.includes('"Origen"')
          ? this.IncapacidadDetalles.origen.id_origen
          : '',
        id_causa: !str.includes('"CausaAtencion"')
          ? this.IncapacidadDetalles.causaAtencion.id_causa
          : '',
        retroactiva: !str.includes('"Retroactiva"')
          ? this.IncapacidadDetalles.retroactiva
          : false,
        id_retroactiva: !str.includes('"Retroactiva"')
          ? this.IncapacidadDetalles.id_retroactiva
          : 0,
        id_trastorno_memoria: !str.includes('"Retroactiva"')
          ? this.IncapacidadDetalles.id_trastorno_memoria
          : 0,
        prorroga: !str.includes('"Prorroga"')
          ? this.IncapacidadDetalles.prorroga
          : 0,
        fecha_inicio_string: !str.includes('"FechaInicio"')
          ? this.IncapacidadDetalles.fecha_inicio_string
          : '',
        dias_incapacidad: !str.includes('"DiasIncapacidad"')
          ? [this.IncapacidadDetalles.dias_incapacidad, Validators.required]
          : '',
        diasAcumuladosProrroga: [''],
        id_incapacidad_anterior: !str.includes('"Prorroga"')
          ? [this.IncapacidadDetalles.id_incapacidad_anterior]
          : '',
      ...
      if (
        ...
        if (this.listaGrupoServicios) {
          this.listaGrupoServiciosPorFiltro = this.listaGrupoServicios.filter(
            (x: any) => x.id_servicio == 3 || x.id_servicio == 5
          );
        }
      }
      ...
      if (
        ...
        this.incapacidadForm.patchValue({
          fecha_inicio_string: !str.includes('"FechaInicio"')
            ? this.IncapacidadDetalles.fecha_expedicion_string
            : '',
        });
      }
      ...
      if (
        this.IncapacidadDetalles.grupoServicios != null &&
        !str.includes('"grupoServiciosId"')
      ) {
      ...
      if (!str.includes('"DiasIncapacidad"')) {
        this.f_incapacidad['dias_incapacidad'].setValue(
          this.IncapacidadDetalles.dias_incapacidad
        );
      ...
      if (!str.includes('"Diagnostico"')) {
        ...
        this.f_incapacidad['descripcion_diagnostico_principal'].setValue(
          this.IncapacidadDetalles.diagnostico.descripcion
        );
      }

      if (
        !str.includes('"DiagnosticoRealacionUno"') &&
        this.IncapacidadDetalles.diagnosticoRelacionUno != null
      ) {
        /*this.f_incapacidad.diagnostico_relacion_1.setValue(parseInt(result.id));*/
        this.diagnosticoRelacionado1CodigoSeleccionado =
          this.IncapacidadDetalles.diagnosticoRelacionUno.cod_diag_relacion_uno;
        this.f_incapacidad['codigo_diagnostico_relacion_1'].setValue(
          this.IncapacidadDetalles.diagnosticoRelacionUno.cod_diag_relacion_uno
        );
        this.f_incapacidad['descripcion_diagnostico_relacion_1'].setValue(
          this.IncapacidadDetalles.diagnosticoRelacionUno.descripcion
        );
      ...
      this.paciente!.tipo_documento =
        this.IncapacidadDetalles.tipoDocumento.cod_tipo_documento;
      this.paciente!.numero_documento =
        this.IncapacidadDetalles.numero_documento_pac;

      this.paciente!.primer_nombre = this.IncapacidadDetalles.primer_nombre_pac;
      this.paciente!.segundo_nombre =
        this.IncapacidadDetalles.segundo_nombre_pac;

      this.paciente!.primer_apellido =
        this.IncapacidadDetalles.primer_apellido_pac;
      this.paciente!.segundo_apellido =
        this.IncapacidadDetalles.segundo_apellido_pac;

      this.f_persona['tipoDocumentoPacienteaBuscar'].setValue(
        this.IncapacidadDetalles.tipoDocumento.cod_tipo_documento
      );
      this.f_persona['numeroDocumentoPacienteaBuscar'].setValue(
        this.IncapacidadDetalles.numero_documento_pac
      );

      if (
        this.IncapacidadDetalles.id_modalidad != null &&
        !str.includes('"Modalidad"')
      ) {
      ...
      let pacienteResult =
        await this.parametrosIncapacidadesService.getPaciente(
          '/Paciente',
          this.IncapacidadDetalles.tipoDocumento.cod_tipo_documento,
          this.paciente!.numero_documento,
          this.IncapacidadDetalles.id_incapacidad
        );
  ...
  async buscarRelacionPaciente() {
    this.loggingService.registroLog(this.paciente);
    let result =
      await this.serviciosIntegracionService.consultarPersonaEnServiciosExternos(
        {
          codigoOperador: '',
    ...
    if (result.relacionPacienteAfiliacionSalud != null) {
      ...
      this.paciente!.id_regimen =
        result.relacionPacienteAfiliacionSalud.regimen;
    ...
    this.paciente!.relacionPacienteAfiliacionSalud =
      result.relacionPacienteAfiliacionSalud;
    this.paciente!.relacionesPacienteAportante =
      result.relacionesPacienteAportante;

    if (this.IncapacidadDetalles.paciente_encontrado == false) {
      this.paciente!.fecha_nacimiento = new Date(
        this.PacienteDetalles.fecha_nacimiento
      );
      this.paciente!.fecha_nacimiento_string =
        this.PacienteDetalles.fecha_nacimiento_string;
    } else {
      this.paciente!.fecha_nacimiento = new Date(result.fecha_nacimiento);
      // let fecha = new Date(this.paciente.fecha_nacimiento_string)
      this.paciente!.fecha_nacimiento_string = this.getFormattedDate(
  ...
  recibiRespuestaEditarPaciente(respuesta: any) {
  ...
  async buscarIncapacidadesAnteriores() {
    ...
    this.listaIncapacidadesAnteriores =
      await this.parametrosIncapacidadesService.postIncapacidad(
        '/Incapacidad/ConsultaPorPaciente',
        valuesParam
      );
    ...
    // WS I
    if (
      this.f_incapacidad['prorroga'].value &&
      this.listaIncapacidadesAnteriores != null
    ) {
      let incapacidadAnteriorValue =
        this.f_incapacidad['id_incapacidad_anterior'].value;
      let index = this.listaIncapacidadesAnteriores.findIndex(
        (x: any) => x.id_incapacidad === incapacidadAnteriorValue.toString()
      );
      if (index != -1) {
  ...
  createIncapacidadForm() {
    ...
    this.incapacidadForm = this.formBuilder.group({
      ...
      fecha_inicio_string: [
        '' /*formatDate(fechaInicial, 'yyyy-MM-dd', 'en')*/,
        Validators.required,
      ],
      dias_incapacidad: [
        0,
  ...
  // convenience getter for easy access to form fields
  get f_incapacidad() {
    return this.incapacidadForm.controls;
  }
  ...
  async buscarPersona() {
    ...
    try {
        ...
        await this.serviciosIntegracionService.consultarPersonaEnServiciosExternos(
          {
            codigoOperador: '',
            tipo_documento:
              this.f_persona['tipoDocumentoPacienteaBuscar'].value,
            numero_documento:
              this.f_persona['numeroDocumentoPacienteaBuscar'].value,
            bolInfoActual: true,
          }
        );
      ...
      if (result.codigo) {
        this.datosDesdeElPadre.deshabilitar =
          this.deshabilitarComponentePaciente = false;
        ...
        await this.Alertas.alertaInformativa(
          'Advertencia',
          'Paciente no encontrado en las bases de datos consultadas',
          'Cerrar'
        );
      } else {
        //Paciente encontrado pero con defuncion
        if (
          result.numeroCertificadoDefuncion != null &&
          result.numeroCertificadoDefuncion != ''
        ) {
          await this.Alertas.alertaInformativa(
            'Advertencia',
            'El paciente se encuentra registrado con certificado de defunción. No se puede expedir la incapacidad.',
            'Cerrar'
          );
          this.spinnerService.hide('spinnerConsultaPaciente');
          return;
        ...
        //Paciente encontrado
        this.datosDesdeElPadre.deshabilitar =
          this.deshabilitarComponentePaciente = true;
        ...
        if (result.relacionPacienteAfiliacionSalud != null) {
          this.paciente!.id_regimen =
            result.relacionPacienteAfiliacionSalud.regimen;
          this.paciente!.eps =
            result.relacionPacienteAfiliacionSalud.codigo_eps;
        }
  ...
  async verificarSeleccion(codigoSeleccionado: any) {
    if (typeof codigoSeleccionado === 'string') {
      this.listaDiagnosticosCie10_principal =
        await this.parametrosIncapacidadesService.get(
          '/Cie10/' + codigoSeleccionado
        );
      let diagnosticoEncontrado = this.listaDiagnosticosCie10_principal.find(
        function (element) {
  ...
  async cambiarBusquedaDiagnostico(search: string) {
    this.listaDiagnosticosCie10_principal =
      await this.parametrosIncapacidadesService.get('/Cie10/' + search);
  }
  ...
  openModalDiagnostico() {
    ...
    modalRef.result.then((result) => {
      if (result) {
        this.f_incapacidad['diagnostico_principal'].setValue(
          parseInt(result.id)
        );
        this.f_incapacidad['codigo_diagnostico_principal'].setValue(
          result.codigo
  ...
  async verificarSeleccionDiagnosticoRelacionado1(codigoSeleccionado: any) {
    if (typeof codigoSeleccionado === 'string') {
      this.listaDiagnosticosCie10_relacion_1 =
        await this.parametrosIncapacidadesService.get(
          '/Cie10/' + codigoSeleccionado
        );
      let diagnosticoEncontrado = this.listaDiagnosticosCie10_relacion_1.find(
        function (element) {
          return element.codigo == codigoSeleccionado;
        }
      );
  ...
  async verificarSeleccionDiagnosticoRelacionado2(codigoSeleccionado: any) {
    if (typeof codigoSeleccionado === 'string') {
      this.listaDiagnosticosCie10_relacion_2 =
        await this.parametrosIncapacidadesService.get(
          '/Cie10/' + codigoSeleccionado
        );
      let diagnosticoEncontrado = this.listaDiagnosticosCie10_relacion_2.find(
        function (element) {
          return element.codigo == codigoSeleccionado;
        }
      );
  ...
  async openModalAportantes() {
    ...
    modalRef.result.then(async (result) => {
      ...
      if (this.paciente!.relacionesPacienteAportante != null) {
        this.paciente!.relacionPacienteAportanteSeleccionada =
          this.paciente!.relacionesPacienteAportante.find(
            (x: any) =>
              x.tipo_documento_ap ==
                this.paciente!.aportanteSeleccionado!.tipo_documento &&
              x.numero_documento_ap ==
                this.paciente!.aportanteSeleccionado!.numero_documento
          ) || null;
      }

      if (
        this.paciente!.aportanteSeleccionado!.tipo_documento != null &&
        this.paciente!.aportanteSeleccionado!.tipo_documento != ''
      ) {
      ...
      this.f_incapacidad['nombre_aportante'].setValue(
        this.paciente!.aportanteSeleccionado!.razon_social
      );

      if (this.paciente!.relacionPacienteAportanteSeleccionada != null) {
        this.f_incapacidad['nombre_arl'].setValue(
          this.paciente!.relacionPacienteAportanteSeleccionada
  ...
  async openModalIncapacidadesAnteriores() {
    ...
    modalRef.componentInstance.dataIn = {
      name: '',
      id: 0,
      listaIncapacidadesAnteriores: this.listaIncapacidadesAnteriores,
    };
    modalRef.result.then(async (result) => {
      this.loggingService.registroLog(result);
  ...
  async calcularNuevasFechaSegunProrrogaSeleccionada(fecha_final_string: any) {
    let nuevaFechaInicioConsecutiva = new Date(fecha_final_string);
    nuevaFechaInicioConsecutiva.setDate(
      nuevaFechaInicioConsecutiva.getDate() + 2
    ); //sumar 1 dia por la conversion y uno mas para el calculo
    this.f_incapacidad['fecha_inicio_string'].setValue(
      this.getFormattedDate(nuevaFechaInicioConsecutiva)
    );
    return true;
  }
  ...
  async incapacidadAnteriorEvent(result: any) {
    if (result.id_incapacidad != null && result.id_incapacidad != 0) {
      ...
      this.f_incapacidad['id_incapacidad_anterior'].setValue(
        result.id_incapacidad
      );
      this.f_incapacidad['descripcion_incapacidad_anterior'].setValue(
        this.getFormattedDateEstandarfromDate(result.fecha_inicio) +
          ' - ' +
          this.getFormattedDateEstandarfromDate(result.fecha_fin) +
          ' - ' +
          result.diagnostico.cod_diagnostico +
          (result.diagnosticoRelacionUno != null &&
          result.diagnosticoRelacionUno.cod_diag_relacion_uno != null
            ? ' - ' + result.diagnosticoRelacionUno.cod_diag_relacion_uno
            : '') +
          (result.diagnosticoRelacionDos != null &&
          result.diagnosticoRelacionDos.cod_diag_relacion_dos != null
            ? ' - ' + result.diagnosticoRelacionDos.cod_diag_relacion_dos
            : '')
      );
      ...
      await this.Alertas.alertaInformativa(
        'Advertencia',
        'Verificar las fechas según la prórroga seleccionada',
        'Cerrar'
      );

      //Validaciones importantes
      await this.calcularNuevasFechaSegunProrrogaSeleccionada(
        result.fecha_fin_string
      );
  ...
  async validarSeleccionGrupoServicioMotivoInternacion(): Promise<boolean> {
    if (
      this.f_incapacidad['id_servicio'].value != '' &&
      this.f_incapacidad['id_retroactiva'].value != '' &&
      (this.f_incapacidad['id_servicio'].value ==
        this._id_servicio_internacion ||
        this.f_incapacidad['id_retroactiva'].value == '1')
    ) {
      if (
        this.f_incapacidad['id_retroactiva'].value == '1' &&
        this.f_incapacidad['id_servicio'].value != this._id_servicio_internacion
      ) {
        return true;
      }
    }
  ...
  async abrirModalConfirmacion(
    modeloIncapacidad: IIncapacidad,
    modeloPaciente: IPacienteModel
  ) {
    ...
    if (this.f_incapacidad['dias_incapacidad'].value == 0) {
      await this.Alertas.alertaInformativa(
        'Advertencia',
        'Se debe indicar el número de días de la incapacidad',
        'Cerrar'
      );
      return;
    }
    ...
    if (
      this.f_incapacidad['prorrogaNoEncontradaHistoricos'].value == true &&
      (this.f_incapacidad['id_incapacidad_anterior'].value == null ||
        this.f_incapacidad['id_incapacidad_anterior'].value == '')
    ) {
    ...
    let validarServicioInternacion =
      await this.validarSeleccionGrupoServicioMotivoInternacion();
    if (validarServicioInternacion) {
      await this.Alertas.alertaInformativa(
        'Advertencia',
        'Grupo de Servicios diferente a Internación',
        'Cerrar'
      );
      return;
    }
    ...
    if (
      this.f_incapacidad['prorroga'].value &&
      this.f_incapacidad['prorrogaNoEncontradaHistoricos'].value == false
    ) {
      if (this.f_incapacidad['diagnostico_principal'].value == '') {
        await this.Alertas.alertaInformativa(
          'Advertencia',
          'Debe seleccionar un diagnóstico principal para continuar',
          'Cerrar'
        );
        return;
      }
      ...
      if (
        this.f_incapacidad['id_origen'].value !=
        this.incapacidadAnterior!.id_origen
      ) {
        await this.Alertas.alertaInformativa(
          'Advertencia',
          'Para la prórroga el origen de la incapacidad actual debe coincidir con el origen de la incapacidad anterior',
          'Cerrar'
        );
        return;
      }
      ...
      if (
        this.f_incapacidad['diagnostico_principal'].value != '' &&
        this.incapacidadAnterior!.diagnostico.id_diagnosticos != null
      ) {
        const coincideDiagnosticoPrincipal =
          this.f_incapacidad['diagnostico_principal'].value != 0 &&
          (this.f_incapacidad['diagnostico_principal'].value ==
            this.incapacidadAnterior!.diagnostico.id_diagnosticos ||
            this.f_incapacidad['diagnostico_principal'].value ==
              (this.incapacidadAnterior!.diagnosticoRelacionUno != null
                ? this.incapacidadAnterior!.diagnosticoRelacionUno
                    .id_diagnosticos
                : 0) ||
            this.f_incapacidad['diagnostico_principal'].value ==
              (this.incapacidadAnterior!.diagnosticoRelacionDos != null
                ? this.incapacidadAnterior!.diagnosticoRelacionDos
                    .id_diagnosticos
                : 0));

        if (
          this.incapacidadAnterior!.consulta == 'bd' &&
          coincideDiagnosticoPrincipal == false
        ) {
          await this.Alertas.alertaInformativa(
            'Advertencia',
            'El diagnóstico de la incapacidad actual no coincide con los diagnóstico principal y relacionados de la incapacidad anterior',
            'Cerrar'
          );
          return;
        }
        ...
        if (this.incapacidadAnterior!.consulta == 'historico') {
          const coincideDiagnosticoRelacion1 =
            this.f_incapacidad['diagnostico_relacion_1'].value != 0 &&
            (this.f_incapacidad['diagnostico_relacion_1'].value ==
              this.incapacidadAnterior!.diagnostico_principal ||
              this.f_incapacidad['diagnostico_relacion_1'].value ==
                this.incapacidadAnterior!.diagnostico_relacion_1 ||
              this.f_incapacidad['diagnostico_relacion_1'].value ==
                this.incapacidadAnterior!.diagnostico_relacion_2);

          const coincideDiagnosticoRelacion2 =
            this.f_incapacidad['diagnostico_relacion_2'].value != 0 &&
            (this.f_incapacidad['diagnostico_relacion_2'].value ==
              this.incapacidadAnterior!.diagnostico_principal ||
              this.f_incapacidad['diagnostico_relacion_2'].value ==
                this.incapacidadAnterior!.diagnostico_relacion_1 ||
              this.f_incapacidad['diagnostico_relacion_2'].value ==
          ...
          if (
            coincideDiagnosticoPrincipal == false &&
            coincideDiagnosticoRelacion1 == false &&
            coincideDiagnosticoRelacion2 == false
          ) {
            await this.Alertas.alertaInformativa(
              'Advertencia',
              'El diagnóstico de la incapacidad anterior no coincide con los diagnóstico principal y relacionados de la incapacidad actual',
              'Cerrar'
            );
            return;
          }
      ...
      const esValido = await this.validacionesProrrogaAdicionales(
        this.incapacidadAnterior
      );
      if (!esValido) {
        return;
      }
    ...
    this.determinarFechaIncapacidadActualEntreRango(
      modeloIncapacidad,
      modeloPaciente
    );
  }

  async validacionesProrrogaAdicionales(result: any) {
    if (this.f_incapacidad['diagnostico_principal'].value == '') {
      await this.Alertas.alertaInformativa(
        'Advertencia',
        'Debe seleccionar un diagnóstico principal para continuar',
    ...
    let calculosdeDiasDiferenciaEntreIncapacidadActualYAnterior =
      this.calcularDiasDiferenciaEntreIncapacidadActualYAnterior(
        new Date(this.f_incapacidad['fecha_inicio_string'].value),
        this.incapacidadAnterior
      );
    if (calculosdeDiasDiferenciaEntreIncapacidadActualYAnterior >= 30) {
      await this.Alertas.alertaInformativa(
        'Advertencia',
        'Prorroga no viable - La fecha fin de la incapacidad anterior supera 30 días' /*'La incapacidad tiene '+ calculosdeDiasDiferenciaEntreIncapacidadActualYAnterior + ' días de diferencia con la anterior'*/,
        'Cerrar'
      );
      return false;
    }
    ...
    fechaFinincapacidadAnterior.setDate(
      fechaFinincapacidadAnterior.getDate() + 1
    );
    let fechaPropectivaMinima = new Date(result.fecha_fin_string);
    fechaPropectivaMinima.setDate(fechaPropectivaMinima.getDate() + 1);

    let diasDiferencia = parseInt(
      Math.ceil(
        (fechaActual.getTime() - fechaPropectivaMinima.getTime()) /
          (1000 * 3600 * 24)
      ) + '',
      10
    );
    ...
    if (diasDiferencia <= -8) {
      await this.Alertas.alertaInformativa(
        'Advertencia',
        'La incapacidad con prórroga prospectiva no puede ser mayor a 8 dias.',
        'Cerrar'
      );
      return false;
    }
  ...
  async guardarIncapacidad(
    ...
    let causaatencionEncontrado = this.listaCausaAtencion.find(function (
      element: any
    ) {
    ...
    let departamentoEncontrado = this.listaDepartamentos.find(function (
      element: any
    ) {
    ...
    this.paciente!.origen_pac_descripcion =
      origenEncontrado != null ? origenEncontrado.descripcion : '';
    this.paciente!.retroactiva_pac_descripcion = modeloIncapacidad.retroactiva
      ? 'Si'
      : 'No';
    this.paciente!.causaatencion_pac_descripcion =
      causaatencionEncontrado != null
        ? causaatencionEncontrado.descripcion
        : '';
    this.paciente!.sexo_descripcion =
      sexoEncontrado != null ? sexoEncontrado.descripcion : '';
    this.paciente!.ciudad_residencia_descripcion =
      ciudadEncontrado != null ? ciudadEncontrado.nom_ciudad : '';
    this.paciente!.depto_residencia_descripcion =
      departamentoEncontrado != null ? departamentoEncontrado.nom_depto : '';
    //this.paciente.regimen_descripcion = (regimenEncontrado!=null)?regimenEncontrado.descripcion:'';

    if (
      modeloIncapacidad.fecha_expedicion_string == null ||
      modeloIncapacidad.fecha_expedicion_string == ''
    ) {
      modeloIncapacidad.fecha_expedicion_string = this.getFormattedDate(
        new Date(Date.now())
      ); //this.getFormattedDateTime(new Date(Date.now()));
    }
    ...
    if (this.f_incapacidad['retroactiva'].value == false) {
      ...
      if (this.fechaExpedicionParametro > fecha_fin) {
        await this.Alertas.alertaInformativa(
          'Advertencia',
          'La fecha fin de incapacidad no puede ser mayor a la fecha de expedición.',
          'Cerrar'
        );
        return;
      }
    ...
    modalRef.componentInstance.dataIn = {
      name: '',
      id: 0,
      incapacidad_data: this.incapacidadForm.value,
      paciente_data: this.paciente,
    };
    await modalRef.result.then(async (result) => {
      ...
      if (result.resultado == 'si') {
        ...
        if (
          (modeloIncapacidad.id_plan_contingencia == null ||
            modeloIncapacidad.id_plan_contingencia == 0) &&
          this.consultaPlanContingenciaActual != null
        ) {
          await this.Alertas.alertaInformativa(
            'Advertencia',
            'Existe un plan de contingencia activo, no se puede expedir la incapacidad',
            'Cerrar'
          );
          return;
        }
        ...
        modeloIncapacidad.numero_documento_pac =
          this.paciente!.numero_documento;
        ...
        modeloIncapacidad.segundo_apellido_pac =
          this.paciente!.segundo_apellido;
        ...
        modeloIncapacidad.lugar_expedicion =
          this.perfilUbicacion.codDepto + this.perfilUbicacion.codMunicipio;
        modeloIncapacidad.lugar_expedicion_descripcion =
          this.lugardeExpedicionEntidad;

        //WS I
        modeloIncapacidad.id_incapacidad_anulado =
          this.paciente!.id_incapacidad_anulado;
        ...
        let result =
          await this.parametrosIncapacidadesService.guardarDatosIncapacidad(
            modelo
          );
  ...
  mostrarOcultarDiagnosticorelacionado1() {
    this.esVisibleDiagnosticorelacionado1 =
      !this.esVisibleDiagnosticorelacionado1;
  }

  mostrarOcultarDiagnosticorelacionado2() {
    this.esVisibleDiagnosticorelacionado2 =
      !this.esVisibleDiagnosticorelacionado2;
  }
  ...
  async calcularFechaFinal() {
    if (
      this.f_incapacidad['dias_incapacidad'].value < 1 ||
      this.f_incapacidad['dias_incapacidad'].value > 30
    ) {
      this.f_incapacidad['dias_incapacidad'].setValue(0);
    }

    if (
      this.f_incapacidad['fecha_inicio_string'].value != null &&
      this.f_incapacidad['dias_incapacidad'].value != null
    ) {
      let fechaInicial = new Date(
        this.f_incapacidad['fecha_inicio_string'].value
      );
      fechaInicial.setDate(fechaInicial.getDate() + 1);
      ...
      if (this.f_incapacidad['retroactiva'].value) {
        if (fechaInicial.getTime() < this.fechaMinimaIncapacidad) {
          await this.Alertas.alertaInformativa(
            'Advertencia',
            'La fecha inicial de la incapacidad no puede ser anterior a mas de 30 días',
            'Cerrar'
          );

          this.f_incapacidad['fecha_inicio_string'].setValue(
            this.getFormattedDate(this.fechaExpedicionParametro)
          );
      ...
      let diasEnMilisegundos =
        (this.f_incapacidad['dias_incapacidad'].value != 0
          ? this.f_incapacidad['dias_incapacidad'].value - 1
          : 0) *
        1000 *
        60 *
      ...
      this.f_incapacidad['fecha_inicio_string'].setValue(
        this.getFormattedDate(fechaInicial)
      );
      ...
      this.loggingService.registroLog(
        this.f_incapacidad['fecha_fin_string'].value
      );
  ...
  async calcularDiasIncapacidadRetroactiva(fechaInico: Date) {
    if (
      this.f_incapacidad['retroactiva'].value &&
      this.f_incapacidad['id_servicio'].value == this._id_servicio_internacion
    ) {
      ...
      if (dias_retroactiva >= 30) {
        await this.Alertas.alertaInformativa(
          'Advertencia',
          'La incapacidad tiene ' + dias_retroactiva + ' días de retroactiva',
          'Cerrar'
        );
      }
    }
  ...
  public calcularDiasDiferenciaEntreIncapacidadActualYAnterior(
    fechaInico: Date,
    incapacidadAnterior: any
  ): number {
    let fechaFinincapacidadAnterior = new Date(
      incapacidadAnterior.fecha_fin_string
    );
    let diff = fechaInico.getTime() - fechaFinincapacidadAnterior.getTime();
    let dias_diferencia_entre_incapacidades = Math.round(
      diff / (1000 * 60 * 60 * 24)
    );
    this.loggingService.registroLog(
      'dias_diferencia_entre_incapacidades:' +
        dias_diferencia_entre_incapacidades
    );
  ...
  async calcularDiasDeProrroga(incapacidadAnterior: any) {
    if (
      ...
      if (incapacidadAnterior.prorroga) {
        this.f_incapacidad['dias_acumulados_prorroga'].setValue(
          incapacidadAnterior.dias_acumulados_prorroga +
            this.f_incapacidad['dias_incapacidad'].value
        );
      }
      //verificar si la anterior incapacidad ya tiene prorroga, sino la tiene debe calcular los dias con los dias de incapacidad
      else {
        this.f_incapacidad['dias_acumulados_prorroga'].setValue(
          incapacidadAnterior.dias_incapacidad +
            this.f_incapacidad['dias_incapacidad'].value
        );
      }
    } else {
  ...
  async changeValueOrigen() {
    this.listaCausaAtencionByOrigen = this.listaCausaAtencion.filter(
      (x: any) => x.id_origen == this.f_incapacidad['id_origen'].value
    );
    this.causaAtencionAnulada == false
      ? this.f_incapacidad['id_causa'].setValue('')
      : '';
    /* this.f_incapacidad.id_causa.setValue('');*/
    ...
    if (this.f_incapacidad['id_origen'].value == 2) {
      ...
      if (
        this.paciente!.aportantes != null &&
        this.paciente!.aportantes.length > 0
      ) {
      ...
      if (
        this.paciente!.aportantes != null &&
        this.paciente!.aportantes.length == 1
      ) {
        this.paciente!.aportanteSeleccionado = this.paciente!.aportantes[0];
        this.paciente!.relacionPacienteAportanteSeleccionada =
          this.paciente!.relacionesPacienteAportante != null
            ? this.paciente!.relacionesPacienteAportante[0]
            : null;
      ...
      if (
        this.proviene_anulado &&
        this.PacienteDetalles.aportanteSeleccionado?.numero_documento != null
      ) {
        this.loggingService.registroLog(this.paciente);
        this.loggingService.registroLog(this.PacienteDetalles);
        ...
        if (
          this.PacienteDetalles.aportanteSeleccionado.tipo_documento != null &&
          this.PacienteDetalles.aportanteSeleccionado.tipo_documento != ''
        ) {
          this.f_incapacidad['id_aportante'].setValue(
            this.PacienteDetalles.aportanteSeleccionado.tipo_documento +
              '-' +
              this.PacienteDetalles.aportanteSeleccionado.numero_documento
          );
        } else {
          this.f_incapacidad['id_aportante'].setValue('');
        }
        this.f_incapacidad['nombre_aportante'].setValue(
          this.PacienteDetalles.aportanteSeleccionado.razon_social
        );
        this.paciente!.aportanteSeleccionado =
          this.PacienteDetalles.aportanteSeleccionado;
        this.paciente!.relacionPacienteAportanteSeleccionada =
          this.paciente!.relacionesPacienteAportante?.find(
            (x: any) =>
              x.tipo_documento_ap ==
  ...
  async changeValueCausaAtencion(e: any) {
    if (
      this.f_incapacidad['retroactiva'].value &&
      this.f_incapacidad['id_retroactiva'].value == 3
    ) {
      if (
        (this.f_incapacidad['id_origen'].value == 1 &&
          this.f_incapacidad['id_causa'].value != 2) ||
        (this.f_incapacidad['id_origen'].value == 2 &&
          this.f_incapacidad['id_causa'].value != 26)
      ) {
        await this.Alertas.alertaInformativa(
          'Advertencia',
          'La causa motivo debe corresponder on el origen',
          'Cerrar'
        );
      }
      return;
  ...
  public seleccionRetroactiva(value: boolean) {
    ...
    if (value) {
      ...
      this.f_incapacidad['fecha_inicio_string'].setValue(
        this.getFormattedDate(fechaDiaAnterior)
      );

      //activar opciones para opcion motivo retroactiva 1.2,4

      if (this.listaMotivaRetroactiva) {
        this.listaMotivaRetroactivaPorFiltro =
          this.listaMotivaRetroactiva.filter(
            (x: any) =>
              x.id_retroactiva == 1 ||
              x.id_retroactiva == 2 ||
              x.id_retroactiva == 4
      ...
      if (this.listaModalidadRealizacionConsulta) {
        this.listaModalidadRealizacionConsultaPorFiltro =
          this.listaModalidadRealizacionConsulta.filter(
            (x: any) => x.id_modalidad == 1
          );
      ...
      if (this.listaModalidadRealizacionConsulta) {
        this.listaModalidadRealizacionConsultaPorFiltro =
          this.listaModalidadRealizacionConsulta;
      }
    }
  }

  public async seleccionProrroga(
    value: boolean,
    validarObedeceProrroga: boolean
  ) {
    if (this.habilitarProrrogaNoEncontradaHistoricosServidor) {
      this.habilitarProrrogaNoEncontradaHistoricosCliente = true;
    }
    ...
    if (validarObedeceProrroga === true) {
      if (
        this.listaIncapacidadesAnteriores != null &&
        this.listaIncapacidadesAnteriores.length == 0
      ) {
        //value = false;
        this.prorrogaSelected = false;
        ...
        await this.Alertas.alertaInformativa(
          'Advertencia',
          'Incapacidad no obedece a prórroga.',
          'Cerrar'
        );
        return;
      } else {
      ...
      this.deshabilitarSeleccionProrroga =
        this.listaIncapacidadesAnteriores != null &&
        this.listaIncapacidadesAnteriores.length > 0
          ? false
          : true;
      ...
      if (!this.tieneProrroga) {
        ...
        this.incapacidadAnterior =
          this.listaIncapacidadesAnteriores != null &&
          this.listaIncapacidadesAnteriores[0] != null
            ? this.listaIncapacidadesAnteriores[0]
            : null;
        this.f_incapacidad['id_incapacidad_anterior'].disable();
  ...
  async determinarFechaIncapacidadActualEntreRango(
    modeloIncapacidad: IIncapacidad,
    modeloPaciente: IPacienteModel
  ) {
    ...
    if (
      ...
      modeloIncapacidad.id_usuario_hercules =
        this.mensajeRegistroIdIncapacidad.idUsuarioEntidadRol;
      modeloIncapacidad.id_plan_contingencia =
        this.mensajeRegistroIdIncapacidad.idPlanContingencia;
    } //Contingencia
    else {
    ...
    if (this.listaIncapacidadesAnteriores != null) {
      this.listaIncapacidadesAnteriores.forEach((elemento: any) => {
        ...
        if (
          ...
          if (
            elemento.diagnostico.id_diagnosticos ==
            this.f_incapacidad['diagnostico_principal'].value
          ) {
    ...
    if (existeIntercepcionFechas) {
      ...
      if (
        existeOrigenComunIntercepcionFechas &&
        this.f_incapacidad['id_origen'].value == 2
      ) {
        await this.Alertas.alertaInformativa(
          'Advertencia',
          'No se puede generar una incapacidad de origen LABORAL cuando existe una incapacidad de origen COMUN vigente',
          'Cerrar'
        );
        return;
      }
      ...
      if (this.f_incapacidad['prorroga'].value) {
        ...
        if (existeMismoDiagnosticoIntercepcionFechas) {
          await this.Alertas.alertaInformativa(
            'Advertencia',
            'Para esta prorroga existe una incapacidad anterior con el mismo diagnóstico principal',
            'Cerrar'
          );
          return;
        }
      ...
      if (!this.f_incapacidad['prorroga'].value) {
        ...
        if (existeMismoDiagnosticoIntercepcionFechas) {
          await this.Alertas.alertaInformativa(
            'Advertencia',
            'Existe una incapacidad anterior con el mismo diagnóstico principal y no requiere otra incapacidad, salvo si procede prórroga',
            'Cerrar'
          );
          return;
        }
        ...
        modalRef.componentInstance.dataIn = {
          name: '',
          id: 0,
          lista_incapacidades: listaIncapacidadesIntercepcion,
          incapacidad_data: this.incapacidadForm.value,
        };
        modalRef.result.then(async (result) => {
          this.loggingService.registroLog(result);
          if (result.resultado == 'si') {
            ...
            if (this.listaIncapacidadesAnteriores != null) {
              this.listaIncapacidadesAnteriores.forEach((elemento: any) => {
                ...
                if (
                  (check_n1 >= from_n && check_n1 <= to_n) ||
                  (check_n2 >= from_n && check_n2 <= to_n)
                ) {
        ...
        this.loggingService.registroLog(
          'hay intercepcion de fechas y hay  prorroga'
        );
        this.guardarIncapacidad(modeloIncapacidad, modeloPaciente);
      }
    } else {
  ...
  public getFormattedDateEstandarfromDate(dateString: string): string {
    if (dateString) {
      ...
      return (
        dateStringDMY.split('-')[2] +
        '/' +
        dateStringDMY.split('-')[1] +
        '/' +
        dateStringDMY.split('-')[0]
      );
    } else {
      return '';
    }
  ...
  async generarPDF() {
    ...
    this.downloadFile(
      valuesParam,
      'certificado_INC' + this.f_incapacidad['id_incapacidad'].value + '.pdf'
    );
  }
  ...
  async cargarListas() {
    try {
      ...
      const fechaServActual = await this.parametrosIncapacidadesService.get(
        '/utilidades'
      );
      this.habilitarProrrogaNoEncontradaHistoricosServidor =
        fechaServActual.realizarRevisionConsultaHistoricos;

      this.listaDepartamentos = await this.parametrosIncapacidadesService.get(
        '/departamentos'
      );
      this.loggingService.registroLog('listaDepartamentos cargada con exito');

      this.listaMunicipios = await this.parametrosIncapacidadesService.get(
        '/municipios'
      );
      this.loggingService.registroLog('listaMunicipios cargada con exito');

      this.listaGrupoServicios = await this.parametrosIncapacidadesService.get(
        '/GrupoServicios'
      );
      ...
      this.listaModalidadRealizacionConsulta =
        await this.parametrosIncapacidadesService.get(
          '/modalidadRealizacionConsulta'
        );
      this.listaModalidadRealizacionConsultaPorFiltro =
        this.listaModalidadRealizacionConsulta;
      this.loggingService.registroLog(
        'listaModalidadRealizacionConsulta cargada con exito'
      );
      ...
      this.listaOrigen = await this.parametrosIncapacidadesService.get(
        '/origen'
      );
      this.loggingService.registroLog('listaOrigen cargada con exito');

      this.listaCausaAtencion = await this.parametrosIncapacidadesService.get(
        '/causaatencion'
      );
      ...
      this.listaMotivaRetroactiva =
        await this.parametrosIncapacidadesService.get('/motivaretroactiva');
      this.listaMotivaRetroactivaPorFiltro = this.listaMotivaRetroactiva;
      this.loggingService.registroLog(
        'listaMotivaRetroactiva cargada con exito'
      );

      this.listaTrastornoMemoria =
        await this.parametrosIncapacidadesService.get('/TranstornoMental');
      this.loggingService.registroLog(
        'listaTrastornoMemoria cargada con exito'
      );

      const perfilUbicacionVar: IPerfilComple =
        JSON.parse(localStorage.getItem('complementario2')!) || '';

      //consultar Lugar de expedicion
      let ciudadExpedicion = await this.listaMunicipios.find(function (
        element: any
      ) {
        return (
          element.cod_municipio == perfilUbicacionVar.codMunicipio &&
          element.cod_depto == perfilUbicacionVar.codDepto
        );
      });

      let departamentoExpedicion = await this.listaDepartamentos.find(function (
        element: any
      ) {
      ...
      this.lugardeExpedicionEntidad =
        (ciudadExpedicion != null ? ciudadExpedicion.nom_ciudad : '') +
        (departamentoExpedicion != null
          ? ' (' + departamentoExpedicion.nom_depto + ')'
          : '');

      this.f_incapacidad['lugar_expedicion'].setValue(
        perfilUbicacionVar.codDepto + perfilUbicacionVar.codMunicipio
      );
      this.f_incapacidad['lugar_expedicion_descripcion'].setValue(
        this.lugardeExpedicionEntidad
      );
      ...
      const listaDiagnosticosAuxiliar =
        await this.parametrosIncapacidadesService.get('/Cie10/' + 'Fiebre');
      ...
      this.loggingService.registroLog(
        'listaDiagnosticosCie10 cargada con exito'
      );

      this.loggingService.registroLog(
        'snpashot:' + this.mensajeRegistroIdIncapacidad?.id
      );
      ...
      if (
        ...
        this.fechaExpedicionParametro = new Date(
          formatDate(fechaServActual.fechaActual, 'yyyy-MM-ddTHH:mm:ss', 'en')
        );
        this.f_incapacidad['fecha_inicio_string'].setValue(
          this.getFormattedDate(this.fechaExpedicionParametro)
        );
        this.f_incapacidad['fecha_expedicion_string'].setValue(
          this.getFormattedDateTime(this.fechaExpedicionParametro)
        );
        ...
        if (this.consultaPlanContingenciaActual == null) {
          await this.Alertas.alertaInformativa(
            'Advertencia',
            'No existe un plan de contingencia activo',
            'Cerrar'
          );
          this.router.navigate(['/menu-principal']);
        }
        ...
        if (this.mensajeRegistroIdIncapacidad.fechaExpedicionForm != '') {
          let fechaExpedicionParametroString = formatDate(
            this.mensajeRegistroIdIncapacidad.fechaExpedicionForm,
            'yyyy-MM-ddTHH:mm:ss',
            'en'
          );
          this.fechaExpedicionParametro = new Date(
            fechaExpedicionParametroString
          );
        } else {
          this.fechaExpedicionParametro = new Date(
            formatDate(fechaServActual.fechaActual, 'yyyy-MM-ddTHH:mm:ss', 'en')
        ...
        if (this.consultaPlanContingenciaActual != null) {
          await this.Alertas.alertaInformativa(
            'Advertencia',
            'Existe un plan de contingencia activo',
            'Cerrar'
          );
          this.router.navigate(['/menu-principal']);
        }
  ...
  inicializarPaciente() {
    const data: IPacienteModel = {
      ...
      sexo_descripcion: '',
      eps_nombre: '',
      relacionPacienteAfiliacionSalud:
        new IRelacionPacienteAfiliacionSaludModel(),
      aportantes: [],
