...
export class ImprimirIncapacidadComponent implements OnInit {
  ...
  get f_informe() {
    return this.formInfomeIncapacidad.controls;
  }

  deshabilitarValidaciones() {
    if (this.formInfomeIncapacidad.get('numeroIncapacidad')?.value != '') {
      this.formInfomeIncapacidad.get('tipoDocumento')?.setValidators(null);
      this.formInfomeIncapacidad.get('numeroDocumento')?.setValidators(null);
      this.formInfomeIncapacidad
        .get('numeroIncapacidad')
        ?.setValidators(Validators.required);
    } else if (this.formInfomeIncapacidad.get('numeroDocumento')?.value != '') {
      this.formInfomeIncapacidad
        .get('tipoDocumento')
        ?.setValidators(Validators.required);
      this.formInfomeIncapacidad
        .get('numeroDocumento')
        ?.setValidators(Validators.required);
      this.formInfomeIncapacidad.get('numeroIncapacidad')?.setValidators(null);
    } else {
      this.formInfomeIncapacidad
        .get('tipoDocumento')
        ?.setValidators(Validators.required);
    ...
    this.formInfomeIncapacidad.get('tipoDocumento')?.updateValueAndValidity();
    this.formInfomeIncapacidad.get('numeroDocumento')?.updateValueAndValidity();
    this.formInfomeIncapacidad
      .get('numeroIncapacidad')
  ...
  async buscarIncapacidadesAnteriores(): Promise<any> {
    ...
    let valuesParam = {
      tipo_documento_pac:
        this.formInfomeIncapacidad.get('tipoDocumento')?.value,
      numero_documento_pac:
        this.formInfomeIncapacidad.get('numeroDocumento')?.value,
      id_incapacidad:
        this.formInfomeIncapacidad.get('numeroIncapacidad')?.value,
  ...
  async buscarInformeIncapacidad(incapacidadIndex: any) {
    const incapacidad: any = {
      ...
      tipoDocumento:
        this.listaIncapacidadesAnteriores[incapacidadIndex].tipo_documento_pac,
      numeroDocumento:
  ...
  async generarPDF() {
    let valuesParam = {
      tipo_documento_pac:
        this.incapacidadResumen.tipoDocumento.cod_tipo_documento,
    ...
    this.downloadFile(
      valuesParam,
      'certificado' + this.incapacidadResumen.id_incapacidad + '.pdf'
    );
  }
