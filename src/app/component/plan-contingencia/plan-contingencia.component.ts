...
export class PlanContingenciaComponent implements OnInit {
  ...
  async cargarDatos() {
    ...
    this.f_planContingencia['fechaRegistro'].setValue(
      new Date(
        formatDate(fechaServActual.fechaActual, 'yyyy-MM-ddTHH:mm:ss', 'en')
      )
    );
  ...
  get f_planContingencia() {
    return this.formPlanContingencia.controls;
  }

  async RegistrarPlanContingencia() {
    this.submittedContingencia = true;
    if (this.formPlanContingencia.valid) {
      var fechaInicioValidacion = new Date(
        formatDate(
          this.f_planContingencia['fechaInicio'].value,
          'yyyy-MM-ddTHH:mm:ss',
          'en'
        )
      );
      ...
      if (
        this.f_planContingencia['fechaFin'].value != null &&
        this.f_planContingencia['fechaFin'].value != ''
      ) {
        var fechaFinValidacion = new Date(
          formatDate(
            this.f_planContingencia['fechaFin'].value,
            'yyyy-MM-ddTHH:mm:ss',
            'en'
        ...
        if (fechaInicioValidacion >= fechaFinValidacion) {
          await this.Alertas.alertaInformativa(
            '',
            'Las fechas inicial o final de la contingencia son inconsistentes. la fecha de inicio debe ser menor a la fecha final.',
            'Cerrar'
          );
          return;
        }
        ...
        if (fechaFinValidacion >= this.fechaHoy) {
          await this.Alertas.alertaInformativa(
            '',
            'Las fechas inicial o final de la contingencia son inconsistentes. la fecha final debe ser menor o igual a la del sistema.',
            'Cerrar'
          );
          return;
        }
      ...
      const lista = this.listaCausalContingencia.filter(
        (causalLista: any) =>
          causalLista.id == this.f_planContingencia['causal'].value
      );
    ...
    if (this.formPlanContingencia.valid) {
      (
        await this.confirmarOperacion(
          'Se registrará contingencia para la entidad ',
          'Actualizar'
        )
      ).subscribe((valid) => {
        if (valid) {
  ...
  async guardarContingencia() {
    ...
    const respuestainsertarPlanContigencia =
      await this.parametrosIncapacidadesService.postIncapacidad(
        '/Incapacidad/CrearPlanContingencia',
        valuesParam
      );
    ...
    if (respuestainsertarPlanContigencia.error == '') {
      ...
      await this.Alertas.alertaInformativa(
        '',
        'Se presento un problema al momento de crear el plan de contingencia ',
        'Cerrar'
      );
      return;
    }
