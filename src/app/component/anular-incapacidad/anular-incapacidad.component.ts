...
export class AnularIncapacidadComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, private parametrosIncapacidadesServiceIn: ParametrosIncapacidadesService, private modal: NgbModal, 
              private router: Router, private spinnerService: NgxSpinnerService,
              cryptoService: CryptoService, accountService: AccountService,
  ...
  async confirmarAnulacion() {
    ...
    try {
      ...
      if(this.consultaPlanContingenciaActual!=null){
        await this.Alertas.alertaInformativa('Advertencia','Existe un plan de contingencia activo, no se puede anular la incapacidad','Cerrar'); 
        return;
      }
  ...
  async cargarListas() {
    ...
    this.consultaPlanContingenciaActual = await this.consultarPlanContingenciaExistente();
    if(this.consultaPlanContingenciaActual!=null){
      await this.Alertas.alertaInformativa('Advertencia','Existe un plan de contingencia activo, no se pueden anular incapacidades','Cerrar'); 
