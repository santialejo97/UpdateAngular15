...
export class LoginComponent implements OnInit {
  ...
  validarUsuarioLogin(datosUsuario: ILogin) {
    this.validarUsuarioService.AutenticarUsuario(datosUsuario).subscribe(
      (data) => {
        const cs: IRetornoUsuario = (this.arreglo = JSON.parse(
          JSON.stringify(data)
        ));
        if (cs.codigo) {
          this.alertService.showError(
            'Error en servicio',
            'Revisar conexión con servicios externos.'
          );
          this.loading = false;
          return;
        }
        ...
        if (cs.loginUsuario.NombreUsuario) {
          this.accountService
            .IniciarSesion(cs.datosUsuario, cs.datosRol)
            .pipe(first())
            .subscribe(
              (data) => {
                this.router.navigate([this.returnUrl]);
          ...
          this.alertService.showError(
            'Autenticación',
            'Los datos de usuario no corresponden'
          );
